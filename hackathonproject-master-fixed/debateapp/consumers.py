...
class DebateConsumer( AsyncWebsocketConsumer ):
    ...
    async def connect( self ):
        ...
        '''
        print("Scope:", self.scope['url_route']['kwargs']['pk'])
        print("AgendaPost.pk", self.scope['url_route']['kwargs']['pk'])
        '''
        # pkを取得して、オブジェクト(討論の空間)ごとにチャットルームを変える
        self.strGroupName = f'chat_{self.scope['url_route']['kwargs']['pk']}'
